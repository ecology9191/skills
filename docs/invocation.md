...
Because `disable-model-invocation: true` excludes a user-invoked skill from the model's available skills, nothing but the human can reach it — no other skill can fire it. So a user-invoked skill may invoke model-invoked skills, but it can never reach another user-invoked skill.
